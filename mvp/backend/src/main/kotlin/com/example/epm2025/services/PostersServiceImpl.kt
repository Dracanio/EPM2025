...
@Service
class PostersServiceImpl( 
    private val postersRepository: PostersRepository,
    private val collaborationRepository: com.example.epm2025.repositories.PosterCollaborationRepository
) : PostersService {
    ...
    override fun getPostersByCollaborator(user: User): List<Poster> {
        val collaborations = collaborationRepository.findByUserId(user.id)
        return collaborations.mapNotNull { it.poster }
    }
}